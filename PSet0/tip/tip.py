...
after dining in a restaurant, typically an amount equal to 15% or 
more of your meal’s cost. Not to worry, though, we’ve written a tip 
...
def dollars_to_float(d):
    d_conversion = d.strip("$")
    return float(d_conversion)


def percent_to_float(p):
    p_conversion = p.strip("%")
    return float(p_conversion)/100
